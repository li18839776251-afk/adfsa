function clearAdText(str) {
    if (!str) return str;
    return str
    .replace(/濂囪抗浜憒-鍔爍缇:锛歖?\s*\d+\s*鑾峰彇鏇村鍏嶈垂璧勬簮/g, "")
    .replace(/[-:]加q.*$/g, "")
    .replace(/[鋦拏糊鈎情垂鍔鐩]+/g, "")
    .replace(/❤️/g, "");
}
...
const newCls = {
    type_id: typeId,
    type_name: "小辛🐉" + clearAdText(typeName || "").replace(/4k-/g, "4K")
};
                    ...
                    if (filter) {
                        ...
                        for (let k = (url) ? 1 : 0; k < filters.length; k++) {
                            ...
                            let typeN = type;
                            if (type.includes("绛涢€ ")) {
                                type = type.replace(/绛涢€ /g, "");
                                if (type === "class") typeN = "绫诲瀷";
                                else if (type === "area") typeN = "鍦板尯";
                                else if (type === "lang") typeN = "璇█";
                                else if (type === "year") typeN = "骞翠唤";
                            }
                            const jOne = {
                                key: type,
                                name: typeN,
        ...
        if (host.includes('/vod') || host.includes('/provide/vod')) {
            ...
            if (obj.list && Array.isArray(obj.list)) {
                for (const item of obj.list) {
                    videos.push({
                        vod_id: item.vod_id,
                        vod_name: (item.vod_name || "").replace(/濂囪抗浜 /g, ''),
                        vod_pic: item.vod_pic || "",
                        vod_remarks: (item.vod_remarks || "").replace(/濂囪抗浜 /g, '')
                    });
                }
            }
            ...
            if (isTV) {
                ...
                for (let i = 0; i < jsonArray.length; i++) {
                    ...
                    const v = {
                        ...
                        vod_name: (vObj.title || "").replace(/濂囪抗浜 /g, ''),
                        vod_pic: vObj.pic,
                        vod_remarks: (vObj.state || "").replace(/濂囪抗浜 /g, ''),
                    };
                    videos.push(v);
                }
            } else {
                ...
                for (const jsonArray of arrays) {
                    for (let i = 0; i < jsonArray.length; i++) {
                        ...
                        const v = {
                            vod_id: vid,
                            vod_name: (vObj.vod_name || "").replace(/濂囪抗浜 /g, ''),
                            vod_pic: vObj.vod_pic,
                            vod_remarks: (vObj.vod_remarks || "").replace(/濂囪抗浜 /g, ''),
                        };
                        videos.push(v);
                    }
                }
...
async function category(tid, pg, filter, extend) {
    try {
        ...
        if (host.includes('/vod') || host.includes('/provide/vod')) {
            ...
            url = url.replace(/绛涢€塩lass/g, extend?.class ?? "");
            url = url.replace(/绛涢€塧rea/g, extend?.area ?? "");
            url = url.replace(/绛涢€塴ang/g, extend?.lang ?? "");
            url = url.replace(/绛涢€墆ear/g, extend?.year ?? "");
            url = url.replace(/鎺掑簭/g, extend?.鎺掑簭 ?? "");
            ...
            if (jsonArray !== undefined) {
                for (let i = 0; i < jsonArray.length; i++) {
                    ...
                    const v = {
                        ...
                        vod_name: ((vObj.vod_name !== undefined ? vObj.vod_name : vObj.title) || "").replace(/濂囪抗浜 /g, ''),
                        vod_pic: vObj.vod_pic !== undefined ? vObj.vod_pic : vObj.pic,
                        vod_remarks: ((vObj.vod_remarks !== undefined ? vObj.vod_remarks : vObj.state) || "").replace(/濂囪抗浜 /g, ''),
                    };
                    videos.push(v);
                }
            }
...
// 杈呭姪鍑芥暟锛氬彧鏇挎崲鏂囧瓧锛屼笉鏀瑰彉鍒嗛殧绗﹀拰缁撴瀯
function replacePlayUrlText(playUrl) {
    if (!playUrl) return playUrl;
    // 浣跨敤姝ｅ垯鍖归厤 $$$ 鎴  # 浣滀负鍒嗛殧绗︼紝浣嗕繚鐣欏師濮嬪垎闅旂涓嶅彉
    // 鍖归厤妯″紡锛氫换鎰忓唴瀹瑰悗璺  $$$ 鎴  #锛屾垨鑰呮湯灏 
    let result = '';
    let i = 0;
    while (i < playUrl.length) {
        ...
        if (found !== -1) {
            ...
            // 鏇挎崲 segment 涓  $ 鍓嶉潰鐨勯泦鏁板悕绉伴噷鐨勬枃瀛 
            ...
            if (dollarInSegment > 0) {
                ...
                episodeName = episodeName.replace(/濂囪抗浜 /g, '');
                result += episodeName + urlPart;
            } else {
                result += segment.replace(/濂囪抗浜 /g, '');
            }
            result += sep;
            i = found + sep.length;
        } else {
...
async function detail(ids) {
    try {
        ...
        if (host.includes('/vod') || host.includes('/provide/vod')) {
            ...
            vod.vod_name = (data.vod_name || "").replace(/濂囪抗浜 /g, '');
            vod.vod_pic = data.vod_pic || "";
            vod.type_name = (data.type_name || "").replace(/濂囪抗浜 /g, '');
            vod.vod_year = (data.vod_year || "").replace(/濂囪抗浜 /g, '');
            vod.vod_area = (data.vod_area || "").replace(/濂囪抗浜 /g, '');
            vod.vod_remarks = (data.vod_remarks || "").replace(/濂囪抗浜 /g, '');
            vod.vod_actor = (data.vod_actor || "").replace(/濂囪抗浜 /g, '');
            vod.vod_director = (data.vod_director || "").replace(/濂囪抗浜 /g, '');
            vod.vod_content = (data.vod_content || "").replace(/濂囪抗浜 /g, '');
            ...
            if (playFrom) {
                ...
                for (let line of lines) {
                    if (line && line.toLowerCase().includes('qijiyun4k')) {
                        newLines.push('小辛专线4k');
                    } else {
                        newLines.push((line || "").replace(/濂囪抗浜 /g, ''));
                    }
                }
            ...
            // 澶勭悊閫夐泦锛氬彧鏇挎崲鏂囧瓧锛屼繚鐣欏師濮嬪垎闅旂鍜岀粨鏋 
            ...
            // 鏇挎崲璇︽儏涓殑鎵€鏈夋枃瀛 
            if (vod.vod_name) vod.vod_name = vod.vod_name.replace(/濂囪抗浜 /g, '');
            if (vod.type_name) vod.type_name = vod.type_name.replace(/濂囪抗浜 /g, '');
            if (vod.vod_year) vod.vod_year = vod.vod_year.replace(/濂囪抗浜 /g, '');
            if (vod.vod_area) vod.vod_area = vod.vod_area.replace(/濂囪抗浜 /g, '');
            if (vod.vod_remarks) vod.vod_remarks = vod.vod_remarks.replace(/濂囪抗浜 /g, '');
            if (vod.vod_actor) vod.vod_actor = vod.vod_actor.replace(/濂囪抗浜 /g, '');
            if (vod.vod_director) vod.vod_director = vod.vod_director.replace(/濂囪抗浜 /g, '');
            if (vod.vod_content) vod.vod_content = vod.vod_content.replace(/濂囪抗浜 /g, '');
...
function getFilterTypes(URL, typeExtend) {
    ...
    if (typeExtend !== null) {
        for (let key in typeExtend) {
            if (key === "class" || key === "area" || key === "lang" || key === "year") {
                try {
                    str += "绛涢€ " + key + "+全部=+" + typeExtend[key].replace(/,/g, "+") + "\n";
                } catch (e) { }
            }
        }
    }
    ...
    if (URL.includes(".vod")) {
        str += "\n" + "鎺掑簭+全部=+鏈€鏂 =time+鏈€鐑 =hits+璇勫垎=score";
    } else if (URL.includes("api.php/app") || URL.includes("xgapp")) {
        // Do nothing, leave the string as it is.
    } else {
        str = "鍒嗙被+全部=+鐢靛奖=movie+杩炵画鍓 =tvplay+缁艰壓=tvshow+鍔ㄦ极=comic+4K=movie_4k+浣撹偛=tiyu+少儿=kid\n绛涢€塩lass+鍏ㄩ儴=+鍠滃墽+鐖辨儏+鎭愭€ +鍔ㄤ綔+绉戝够+鍓ф儏+鎴樹簤+璀﹀尓+鐘姜+鍔ㄧ敾+濂囧够+姝︿緺+鍐掗櫓+鏋垬+鎭愭€ +鎮枒+鎯婃倸+缁忓吀+闈掓槬+鏂囪壓+寰數褰 +鍙よ+鍘嗗彶+杩愬姩+鍐滄潙+鎯婃倸+鎯婃倸+浼︾悊+鎯呰壊+绂忓埄+涓夌骇+鍎跨+缃戠粶鐢靛奖\n绛涢€塧rea+鍏ㄩ儴=+澶ч檰+棣欐腐+鍙版咕+缇庡浗+鑻卞浗+娉曞浗+鏃ユ湰+闊╁浗+寰峰浗+娉板浗+鍗板害+瑗跨彮鐗 +鍔犳嬁澶 +鍏朵粬\n绛涢€墆ear+鍏ㄩ儴=+2025+2024+2023+2022+2021+2020+2019+2018+2017+2016+2015+2014+2013+2012+2011+2010+2009+2008+2007+2006+2005+2004+2003+2002+2001+2000";
    }
...
function getCateFilterUrlSuffix(URL) {
    if (URL.includes("api.php/app") || URL.includes("xgapp")) {
        return "&class=绛涢€塩lass&area=绛涢€塧rea&lang=绛涢€塴ang&year=绛涢€墆ear&limit=18&pg=#PN#";
    } else if (URL.includes(".vod")) {
        return "&class=绛涢€塩lass&area=绛涢€塧rea&lang=绛涢€塴ang&year=绛涢€墆ear&by=鎺掑簭&limit=18&page=#PN#";
    } else {
        return "&page=#PN#&area=绛涢€塧rea&type=绛涢€塩lass&start=绛涢€墆ear";
    }
}
...
function isBan(key) {
    return key === "浼︾悊" || key === "鎯呰壊" || key === "绂忓埄";
}
...
async function searchContent(key, pg) {
    let res = await req.get(`http://cmsyun2.qijiyun.vip/api.php/app/?pg=${pg}&wd=${encodeURIComponent(key)}`);
    let jsonArr = JSON.parse(res).list;
    const result = { list: [] };
    if (jsonArr != null) {
        for (let i = 0; i < jsonArr.length; i++) {
            const jObj = jsonArr[i];
            if (isBan(jObj.type_name)) continue;
            const vodName = clearAdText(jObj.vod_name || "").replace(/4k-/g, "4K");
            result.list.push({
                vod_id: jObj.vod_id,
                vod_name: "小辛🐉" + vodName,
                vod_pic: jObj.vod_pic,
                vod_remarks: clearAdText(jObj.vod_remarks || "")
            })
        }
    }
    return result;
